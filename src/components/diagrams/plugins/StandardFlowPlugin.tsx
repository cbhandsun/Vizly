...
import type { DiagramTypePlugin, PluginContext, SidebarPanel } from '@vizly/core/types';
...
import { getCustomPreset } from '@vizly/core/storage';
...
const FlowchartShapesPanel = React.lazy(() => import('@vizly/core/flowchart-tools').then(module => ({ default: module.FlowchartShapesPanel })));
const JsonEditorModal = React.lazy(() => import('@vizly/core/flowchart-tools').then(module => ({ default: module.JsonEditorModal })));
...
export class StandardFlowPlugin implements DiagramTypePlugin {
  ...
  async onAIAction(action: string, params: unknown, ctx: PluginContext): Promise<boolean> {
    ...
    switch (action) {
      case 'smart-optimize':
        // 插件特定的智能优化逻辑
        import('@vizly/core/layout').then(({ recommendLayout }) => {
