...
import {
    ...
    readActiveDiagramNodeCount,
} from '../diagramViewerSwitchGuard';

describe('diagramViewerSwitchGuard', () => {
    it('reads the active scoped canvas and fails closed when it is unavailable', () => {
        const lookup = vi.fn((id: string) => id === 'active'
            ? { nodes: [{ id: 'edited' }] }
            : undefined);

        expect(readActiveDiagramNodeCount('active', lookup)).toBe(1);
        expect(lookup).toHaveBeenCalledWith('active');
        expect(() => readActiveDiagramNodeCount('missing', lookup)).toThrow('unavailable');
        expect(() => readActiveDiagramNodeCount('', lookup)).toThrow('unavailable');
        expect(() => readActiveDiagramNodeCount('invalid', () => ({ nodes: undefined }))).toThrow('unavailable');
    });
