...
describe('diagramViewerAiBridge', () => {
    ...
    it('binds newly generated diagrams to the application target id', () => {
        expect(bindAIDiagramToTargetId({ id: 'model-chosen-id', nodes: [] }, 'app-target-id')).toEqual({
            id: 'app-target-id',
            nodes: [],
        });
        expect(() => bindAIDiagramToTargetId([], 'app-target-id')).toThrow('must be an object');
    });
    ...
    it('imports AI JSON into the active bridge for preview and apply modes', () => {
        ...
        expect(importData).toHaveBeenNthCalledWith(1, expect.any(Object), { keepHistory: true, mode: 'append' });
        expect(importData).toHaveBeenNthCalledWith(2, expect.any(Object), { keepHistory: true, mode: 'append' });
    });
