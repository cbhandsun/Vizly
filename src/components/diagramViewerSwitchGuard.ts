import { appModal } from '@vizly/core/antd';
import { COMMERCIAL_VIEWPORT_MODAL_CLASS } from '@vizly/core/viewport';
...
export const readActiveDiagramNodeCount = (
    diagramId: string,
    lookupBridge: (id: string) => FlowDataBridgeEntry | undefined = getFlowDataBridge,
): number => {
    const bridge = diagramId ? lookupBridge(diagramId) : undefined;
    if (!bridge || !Array.isArray(bridge.nodes)) {
        throw new Error('Active diagram canvas is unavailable');
    }
    return bridge.nodes.length;
};
