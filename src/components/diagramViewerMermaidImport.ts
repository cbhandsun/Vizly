...
interface ImportMermaidGraphToDiagramOptions {
    diagramId: string;
    nodes: unknown;
    edges: unknown;
    scheduleSmartLayout?: () => void;
}
...
const scheduleDefaultSmartLayout = () => {
    setTimeout(() => {
        window.dispatchEvent(new CustomEvent('editor:command', { detail: { action: 'smart-layout' } }));
    }, 500);
};

export const importMermaidGraphToDiagram = async ({
    diagramId,
    nodes,
    edges,
    scheduleSmartLayout = scheduleDefaultSmartLayout,
}: ImportMermaidGraphToDiagramOptions): Promise<boolean> => {
    const bridge = getFlowDataBridge(diagramId);
    const addNode = bridge?.addNode;
    if (!bridge || !addNode) return false;

    await importMermaidGraphToBridge({
        bridge: { addNode, connectNodes: bridge.connectNodes },
        nodes,
        edges,
    });
    scheduleSmartLayout();
