...
describe('aiCommandPolicy', () => {
    ...
    it('validates command payload boundaries', () => {
        expect(validateAutonomousAICommand({ action: 'addNode' }).allowed).toBe(false);
        expect(validateAutonomousAICommand({ action: 'addNode', label: '   ' }).allowed).toBe(false);
    ...
    it('requires bounded plain-object theme updates', () => {
        ...
        expect(validateAutonomousAICommand({
            action: 'updateTheme',
            style: { 'node-bg': '#fff', 'node-border': '#222' },
        }).allowed).toBe(true);
    });
