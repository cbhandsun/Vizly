...
const hasSafeStyle = (value: unknown): value is Record<string, string> => {
    if (!isPlainRecord(value)) return false;
    const entries = Object.entries(value);
    return entries.length > 0
        && entries.length <= MAX_STYLE_KEYS
        && entries.every(([key, styleValue]) => (
        key.length > 0
        && key.length <= MAX_ACTION_LENGTH
        && !BLOCKED_STYLE_KEYS.has(key)
        && SAFE_STYLE_KEY_PATTERN.test(key)
        && typeof styleValue === 'string'
        && styleValue.length <= MAX_STYLE_STRING_LENGTH
    ));
};
...
export const validateAutonomousAICommand = (cmd: unknown): AICommandPolicyResult => {
    ...
    switch (action) {
        case 'addNode':
            if (!hasBoundedString(command.label, MAX_LABEL_LENGTH)) {
                return { allowed: false, reason: 'missing or invalid label' };
            }
