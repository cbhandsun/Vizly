...
export const cloneDisplayRoutingWorkerSessionRef = (
  workerSessionRef: RoutingWorkerSessionRef,
): RoutingWorkerSessionRef => Object.freeze({
  sessionId: workerSessionRef.sessionId,
  identity: createDisplayRoutingIdentity(
    workerSessionRef.identity.inputSignature,
    workerSessionRef.identity.inputGeometryDigest,
  ),
...
export const isDisplayRoutingIdentity = (value: unknown): value is RoutingIdentity => {
  ...
  return Object.keys(identity).length === 4
    && identity.routingVersion === EDGE_ROUTING_CACHE_VERSION
    ...
    && typeof identity.inputGeometryDigest === 'string'
    && GEOMETRY_DIGEST_PATTERN.test(identity.inputGeometryDigest);
};
...
export const isDisplayRoutingWorkerSessionRef = (
  ...
  return Object.keys(ref).length === 3
    ...
    && typeof ref.outputRouteSignature === 'string'
    && OUTPUT_ROUTE_SIGNATURE_PATTERN.test(ref.outputRouteSignature);
};
