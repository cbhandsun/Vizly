...
const nodes: Node[] = [
  { id: 'hub', position: { x: 0, y: 0 }, measured: { width: 100, height: 50 }, data: {} },
  { id: 'a', position: { x: 200, y: 0 }, measured: { width: 100, height: 50 }, data: {} },
  { id: 'b', position: { x: 220, y: 10 }, measured: { width: 100, height: 50 }, data: {} },
  { id: 'far', position: { x: 1000, y: 0 }, measured: { width: 100, height: 50 }, data: {} },
  { id: 'down', position: { x: 0, y: 300 }, measured: { width: 100, height: 50 }, data: {} },
];
const edges = (...items: Edge[]): Edge[] => items;
...
describe('AdvancedRouting', () => {
  ...
  it('adds layer control points to long edges', () => {
    const result = layerBasedEdgeRouting(edges(
  ...
  it('places labels and adjusts overlapping labels away from nodes or prior labels', () => {
    const result = optimizeEdgeLabelPositions(edges(
  ...
  it('creates tree bus paths for one-to-many and many-to-one groups', () => {
    const oneToMany = optimizeTreeBusRouting(edges(
      { id: 'e1', source: 'hub', target: 'a' },
      { id: 'e2', source: 'hub', target: 'b' },
    ...
    const oneToManyData = oneToMany[0].data as { computedPath?: unknown[] };
    expect(oneToManyData.computedPath).toHaveLength(4);

    const manyToOne = optimizeTreeBusRouting(edges(
