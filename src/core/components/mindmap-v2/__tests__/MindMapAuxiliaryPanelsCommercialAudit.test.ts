...
describe('mind map auxiliary panel commercial audit contract', () => {
    ...
    it('uses a named, confirmed, top-layer restore action for each history snapshot', () => {
        ...
        expect(source.match(/placement="bottomRight"/g)).toHaveLength(2);
        expect(source).toContain('autoAdjustOverflow');
        expect(source).not.toContain('autoAdjustOverflow={false}');
        expect(source).toContain("maxWidth: 'min(540px, calc(100vw - 80px))'");
        expect(source).toContain('getPopupContainer={getViewportPopupContainer}');
        expect(source).toContain('useMindMapHistoryConfirmationFocus(');
        expect(source).toContain("cancelButtonProps={{ id: getMindMapHistoryConfirmCancelId('clear') }}");
        expect(source).toContain('cancelButtonProps={{ id: getMindMapHistoryConfirmCancelId(r.id) }}');
        expect(source).toContain('appMessage.success(');
        expect(source).not.toContain('import { Popconfirm, message }');
