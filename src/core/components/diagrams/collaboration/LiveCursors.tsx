...
export interface LiveCursorsProps {
  activeUsers: unknown;
  yAwareness?: CollaborationAwarenessLike | null;
}
...
export const LiveCursors: React.FC<LiveCursorsProps> = ({ activeUsers, yAwareness }) => {
  ...
  useEffect(() => {
    ...
    const syncCursor = () => {
      if (hasPendingUpdate) {
        yAwareness.setLocalStateField('cursor', targetPosition);
    ...
    const handlePointerMove = (event: PointerEvent) => {
      targetPosition = reactFlow.screenToFlowPosition({ x: event.clientX, y: event.clientY });
  ...
  return (
    ...
    >
      {safeActiveUsers.map((presence) => {
        if (!presence.cursor || presence.clientId === yAwareness?.clientID) return null;
        const x = presence.cursor.x * transform[2] + transform[0];
        const y = presence.cursor.y * transform[2] + transform[1];
        ...
        return (
          ...
          >
            <CursorIcon color={presence.user.color} />
            <div
