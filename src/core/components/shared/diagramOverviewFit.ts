...
import { computeDiagramFitViewport, resolveDiagramFitLayout, type DiagramFitViewport, type DiagramFitViewportInput } from './diagramControlFit';
...
export const readDiagramOverviewFitInput = ({ container, nodes, edges, viewport, fallbackSize }: {
  ...
  const bounds = computeDiagramContentBounds(nodes, edges, canvas ? readDiagramRenderedLabelBounds(canvas, viewport) : []);
  ...
  return { bounds, viewportWidth: width, viewportHeight: height, ...resolveDiagramFitLayout({
...
export const applyDiagramOverviewFit = async ({
  ...
  const target = input ? computeDiagramFitViewport(input) : null;
  ...
  const correction = computeDiagramFitViewport(measuredInput);
