...
export const createDisplayCrossingClusterMazePool = (
...
): { edgeIndexes: readonly number[]; get: (edgeIndex: number) => Edge | null } => {
  const pairs = new Map<number, readonly [DisplaySegment, DisplaySegment]>();
  for (const hit of firstDisplayCrossingClusterStrictHits(segments)) {
  ...
  const get = (edgeIndex: number): Edge | null => {
  ...
  return { edgeIndexes: [...pairs.keys()], get };
};
