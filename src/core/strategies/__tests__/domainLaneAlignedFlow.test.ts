...
describe('shared process ranks with local branch separation', () => {
  ...
  it.each(cases)('preserves business order and full routing quality in $name $direction', async ({ name, preset, direction, productionGeometry }) => {
    ...
    const options = {
      ...
      spacing: { horizontal: 120, vertical: 120 }, padding: { top: 40, right: 20, bottom: 20, left: 20 },
    ...
    const projected = projectBaseReactFlowDisplayWorkerInput({ nodes: original.nodes, edges: sourceEdges });
    const seed = seedBaseReactFlowStagedLayoutEdges({ sourceEdges: projected.edges, sourceNodes: projected.nodes });
    const unseeded = sourceEdges.map(edge => ({ ...edge, data: clearBaseReactFlowLayoutEdgeRoutingData(edge.data) }));
    const request = {
      operation: 'repair-validate-or-route', requestId: 'aligned-lanes', nodes: projected.nodes, edges: unseeded,
      fallbackCandidateEdges: seedBaseReactFlowStagedLayoutEdges({ sourceEdges: unseeded, sourceNodes: projected.nodes }),
