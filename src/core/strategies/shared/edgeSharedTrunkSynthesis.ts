...
const MIN_BRANCH_SPAN = 24;
const MIN_PARALLEL_GAP = 8;
const MIN_ENDPOINT_TAIL = 48;
...
function branchValue(path: Point[], side: Side, endpoint: EndpointKind): number | null {
  ...
  if (endpoint !== 'source' || path.length < 2) return null;
  const adjacent = path[1];
...
function signDelta(delta: number): number {
...
function applySharedEndpointTrunks(edges: Edge[], endpoint: EndpointKind): Edge[] {
...
export function synthesizeSharedEndpointTrunks(edges: Edge[]): Edge[] {
  return refineSourceBranchLanesByDirection(
    applySharedEndpointTrunks(
      applySharedEndpointTrunks(edges, 'source'),
      'target',
    ),
  );
}

export function synthesizeSharedTargetTrunks(edges: Edge[]): Edge[] {
  return applySharedEndpointTrunks(edges, 'target');
}

export const synthesizeSharedSourceTrunks = synthesizeSharedEndpointTrunks;
