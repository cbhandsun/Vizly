...
export const logEdgeRoutingCoordinatorParallelPoolInitFailure = (error: unknown): void => {
...
export const logEdgeRoutingCoordinatorCachesCleared = (graphVersion?: number): void => {
  ...
  safeLog.info('[EdgeRoutingCoordinator] All caches cleared. Edges will re-route.');
};

export const logEdgeRoutingCoordinatorNoLatestRequest = (targetId: string): void => {
  safeLog.warn('[EdgeRoutingCoordinator] forceDebugReRoute: no latest request for edge', targetId);
};

export const logEdgeRoutingCoordinatorSerialRoutingFailure = (edgeId: string, error: unknown): void => {
  safeLog.error(`[Coordinator] Serial routing failed for ${edgeId}:`, redactSensitiveLogValue(error));
};

export const logEdgeRoutingCoordinatorBatchRoutingFailure = (error: unknown): void => {
  safeLog.error('[EdgeRoutingCoordinator] batchRouteDirtyEdges failed:', redactSensitiveLogValue(error));
};

export const logEdgeRoutingCoordinatorMissingResult = (edgeId: string, index: number): void => {
  safeLog.error(`[Coordinator] Missing result for edge ${edgeId} at index ${index}`);
};

export const logEdgeRoutingCoordinatorParallelIncomplete = (expected: number, actual: number | undefined): void => {
  safeLog.error(
    `[EdgeRoutingCoordinator] Parallel routing returned incomplete results. Expected ${expected}, got ${actual}`
  );
};

export const logEdgeRoutingCoordinatorParallelFallback = (error: unknown): void => {
  safeLog.error('[P0 Parallel] Failed, falling back to serial:', redactSensitiveLogValue(error));
};

export const logEdgeRoutingCoordinatorGlobalNudgeFailure = (error: unknown): void => {
  safeLog.error('[GlobalNudge] Channel routing failed, falling back to original paths:', redactSensitiveLogValue(error));
};

export const logEdgeRoutingCoordinatorDebugToolsReady = (): void => {
  safeLog.info('[Vizly Dev] Routing debug tools available: window.__vizly_routing__.clearCache()');
};
