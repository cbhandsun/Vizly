...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地。四图各 16 项布局已有历史连线检查覆盖，指定短视口跨引擎序列已通过；新增四图指定主链 TB/LR/BT/RL/TB 共 20 次浏览器语义验收通过。候选预算减算 `a085c4d0` 完整 CI `33424639344` 成功；语义验收 `7145a275` 已推送，完整 CI 尚在运行、性能专项失败。普通自动保存后刷新已复现质量拒绝，不能用 canonical 标准图通过替代保存图验收。正式性能、剩余业务语义/任意跨引擎切换及导出完整验收仍未完成。
