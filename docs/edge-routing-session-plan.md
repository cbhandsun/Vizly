...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；`3ccc47f0` 的远端完整 CI 已通过。结合此前证据，四图各 16 项布局均已有连线检查覆盖。高级子菜单越出视口已本地修复；更严格的真实点击和短视口连续切换检查发现 Demand/TMS 的独立最终几何问题。正式冷路由性能、跨域语义、保存图/跨引擎完整连续切换和导出验收仍未完成。
