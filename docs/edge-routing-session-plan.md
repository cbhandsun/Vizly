...
#### 当前有效结论（以此节覆盖下方历史批次的待办状态）

- 已交付提交：`3af61cfd`（反向布局夹具随缓存 v16 重绑）。完整 CI `33940945117` 成功：静态、生产构建、桌面/移动 smoke、普通保存恢复、五组测试及覆盖率汇总全部通过。BT/RL 两份夹具只更新版本和输入身份；新增显式身份断言，节点、边、路径及 30 秒事务期限不变。
- 未完成项：性能专项 `33940945112` 的冷路由失败，10 个独立 Logistics 样本 route 中位 1135ms、p95 1342/1100ms，Worker 中位 1114.8ms、p95 1331.6ms，额外开销中位 11.8ms、p95 38.2ms。增量及交互专项通过。上一提交 route p95 1495ms 与本次差异不是算法收益（生产算法未变）；不得用完整 CI 成功替代冷路由性能通过。
- 本轮排除的低收益方向：折线修复 10 次调用无完整输入重复，14038 个候选只有 135 次进入候选质量评估；生成、快照、避障和交叉已有缓存/提前筛选。稀疏边对汇总试验通过 22 项测试，但本地替换评分累计约 101ms，对照约 92ms，未证明收益，已完全撤回。诊断结果只用于选择下一步，不作为生产提速或完整门禁证据。
- 用户已明确同意冷启动性能单列下一迭代，不再阻塞本轮已验证功能收口。该项仍未完成，1100ms 门禁、专项工作流和失败记录全部保留；本次只调整迭代归属，不修改脚本、预算、测试或算法，不将失败改称通过。
- 下一迭代的冷路由验收：先取得算法级工作量证据，再做同构建环境的前后对照；完整连线质量、缓存身份、最终路径授权和产物独立复现不退化，最后执行既有 10 个独立冷样本专项并满足 1100ms p95。停止已排除的微优化和零碎 bundle 压缩，不无限重复相同试验。3D 仅性能继续按用户授权后置；更紧凑的分段/折叠泳道属于后续显式视图能力，不能破坏现有同组等宽/等高原则。

以下为历史批次记录，出现“尚未提交”“等待 CI”等描述时不代表当前状态。

- 缓存发布闭环补齐：发现持久化路由 namespace 和输入身份使用 `EDGE_ROUTING_CACHE_VERSION`，仅更新 manifest 源码签名不能让版本 15 的本地候选自然失效。本批提升至 16，新增旧版候选拒绝回归；四项产物按捕获构建→生成→纳入产物重建→独立 production 重放顺序验证，四份 patches 与版本 15 逐字段一致。三份历史 Worker 请求样本只重算版本/输入身份，节点、边、路径不改；缓存注册、旧版拒绝、WMS 冷路由、预编译幂等、需求分配及泳道回归通过。不会删除用户业务图表，不把缓存失效当作冷启动性能改善。

- `6113be21` 验收结果已收敛：CI `33938870952` 在一次原配置 UI 复验后整体成功，包含全部测试组、静态/构建、桌面/移动 smoke 与普通保存恢复。Routing performance `33938870812` 的增量及交互专项成功；冷启动复验成功进入全部 10 个独立样本，但 Logistics route 中位 1107ms、p95 1520/1100ms，Worker 中位 1095.8ms、p95 1506ms，route overhead 中位 11.7ms、p95 18.9ms。本轮剩余为实际计算预算未达标，不再归因于线程投递或 Chrome 启动；保留失败，不重复重跑争取偶然绿灯。下一步只考虑有明确算法级收益的计算优化，不继续无稳定收益的局部缓存实验；功能交付与剩余性能项分开报告。

- 外侧端口缓存实验已撤回，未进入生产提交。虽然 13,899 次障碍检测只有 6,370 组路径/边身份，现有分段缓存复用与原实现逐次结果、完整候选顺序和元数据均一致，但三组同进程配对生成耗时为 fresh/cached `270.2/257.6`、`217.7/222.6`、`183.8/201.7ms`，没有稳定收益，不继续投入产物同步或微优化。相关临时验证不作为发布门禁通过记录。
- `6113be21` 远端首次 CI：静态/构建/smoke、routing、flow、foundation 已通过；UI 唯一失败为 ShareDialog 非法邮箱测试 5 秒超时，本地原配置完整 27 项通过，因此没有改业务逻辑或超时，已做一次原配置失败任务复验。冷启动任务在第一个样本之前因 Chrome DevTools 15 秒启动超时失败（进程已输出 listening、HTTP probe 尚 pending），不是该提交的业务 p95 结论；同样仅做一次原配置复验，保留初次失败记录。

- 本批发布验证：统一 `test:changed:routing` 三组共 80 文件/600 测试通过（521+45+34）；WMS 冷路由专项独立通过，26 项质量/捷径/泳道回归通过。全库 Lint 零错误/零历史警告，类型、严格 Core、TS6、架构、源码规模、安全、CI 收录、生产构建和经用户审查后的 bundle 门禁通过。四项生产预编译路径已独立复现。本批提交只包含净距候选分阶段、对应回归及产物；剩余主要项是最新完整远端 CI 和 Logistics 冷启动尾延迟，上轮远端为 p95 1896/1100ms，增量与交互专项通过。3D 性能仍后置，不把本批验证当作整个计划完成。
...
- 已确认 main 的 manifest 仍记录旧路由源码签名，工作区与 Git 源码签名一致；`4b3e0391` 后四项生产产物需要同步。当前已完成四项重新生成及独立 `--check` 重放，包含 WMS 横向泳道新的几何身份；这证明产物可复现，不代表全库路由质量已通过。
- 下一优先项：远端 `33933677226` 的 WMS 全流程质量用例报告 3 处最小净距违规；另有 WMS 冷路由精确指纹变化及阶段统计断言失配。先修真实净距问题，再核对其他断言，保留所有质量预算。冷路由 p95、最新完整 CI 仍未闭环；3D 性能继续后置。
...
## 56. 泳道密度边界与稳定布局状态门禁（2026-09-04）

用户最新局部截图再次显示纵向泳道存在大块稀疏区域。当前 production 页面复验确认 `理论水位计算 → 需求排序 → 水位阈值判定` 已按业务方向正向排列，条件标签也贴在最终授权路径附近；剩余长度来自 21 个连续业务层级共享同一流程轴，而不是标签旧坐标或节点逆序。现有纵向空白带已经按整组统一压到 `64px`，继续机械缩小会重新引入已在真实 LR/TB 矩阵中出现过的端点锚定和最小净距失败。后续若继续提升密度，应新增显式的分段泳道或折叠子流程视图：所有泳道在同一阶段边界同步折叠，保持同组等宽/等高；默认严格泳道仍保留连续时间轴语义。该能力不得通过逐泳道裁切或继续压缩节点可读空间实现。

前一提交的远端五组测试和覆盖率全部通过，最终失败仅发生在多页恢复后验证布局选择：节点、页面持久化、26 条边、最终授权路径和完整几何质量均通过，但门禁通过翻译后的工具栏 `aria-label` 反推当前布局，React 页面切换提交期间偶发只能读到基础“自动布局”文案并被判为 `unrecognized`。工具栏现在额外暴露受控的稳定布局 key；浏览器门禁优先验证该 key，缺失时仍兼容原文案匹配，并对任何已知但错误的布局立即失败。回归覆盖翻译文案未提交、稳定 key 正确、稳定 key 错误和非法输入；1024×600 production 多页新增、复制、两种布局、重载及往返矩阵连续两轮通过，所有节点/边数量、页面布局、标记隔离、障碍、净距、正交性、端点和渲染授权保持通过。

## 57. WMS 跨布局切换中的重复容器边界（2026-09-05）

WMS 经过 compound ELK BT → 泳道 RL → full ELK LR → 泳道 BT 后，最后一步被质量门禁拒绝。冷启动直接选择 BT 能通过。两份请求的 39 个业务节点绝对几何和 44 条主候选路径一致；失败请求多出 6 个嵌套视觉分组。净距修复把这些更小的分组边界重复当作不可越出的走线约束，挡住安全候选。

修复仅合并几何包含且业务节点集合相同、非空的重复容器约束。真正划分不同业务节点的子域继续保留边界；画布容器、层级和泳道尺寸不变。无图或节点 ID 特判。直接忽略全部嵌套容器的试验导致需求分配预编译商业检查失败，已撤换。

验证：真实 WMS 请求回归通过，浏览器四步切换均 committed/final-applied，各步 hardClean=true、最小及商业净距违规为 0；4 个正式预编译场景通过且产物几何不变。verify:static（含生产构建、类型、Lint、安全、架构与 bundle）、TS6 和 1100 项测试文件 CI 收录检查通过。相关 jsdom 回归为 9 通过、1 失败：既有 bounded demand-allocation 用例在原实现和本次实现上均 hardClean=false，路由指标一致（40 折、长度 15873）；这项仍需单独修复，不将整组测试报告为通过。本批不代表整体布局密度、标签缩放或全部 I5 验收完成。
