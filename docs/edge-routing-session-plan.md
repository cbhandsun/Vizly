...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；本批本地公共寻路 198 项、Logistics 34 项及冷路由 10 项通过，修复此前远端质量回归，待本批远端完整 CI 验证。TMS 同源码曾单独超时，正式性能稳定性仍未闭环；WMS Demand BT、跨域全局语义排序和最终完整矩阵继续推进。
