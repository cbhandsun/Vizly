...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地。四图各 16 项布局均已有历史连线检查覆盖；四图短视口跨引擎指定四步序列也已通过。负耗时采样修复已推送为 `e1468074`；随后主题入口依赖修复的本地首屏/静态验收通过。最近 `edf7f3a4`、`691fe244` 的五组测试及覆盖率通过，但首屏资源预算导致完整 CI 失败；不能沿用旧提交全绿结论。正式冷路由性能、跨域语义、保存图/任意跨引擎切换和导出完整验收仍未完成。
