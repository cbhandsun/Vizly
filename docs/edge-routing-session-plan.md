...
- 执行边界：停止上述微优化及零碎 bundle 压缩；冷路由仍保留 1100ms 门禁，不能跳过搜索、降低连线质量或将其擅自后置。若无可验证的算法级收益，应与用户确认性能项的迭代归属，不无限重复相同试验。3D 仅性能继续按用户授权后置；更紧凑的分段/折叠泳道属于后续显式视图能力，不能破坏现有同组等宽/等高原则。
