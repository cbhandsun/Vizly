...
- `4d2ce3bb` 已推送显式泳道失败回滚回归；本地对应 CI 整组 98 文件、594 项通过。远端类型门禁发现测试错误访问 React setter 的 mock 内部字段，后续改用等价调用断言并补跑类型检查；最新 main 的完整 CI 仍待确认。

1. **稳定修复交付**：泳道修复已推送；优先交付增量采样误报修复，定位 core 与 3D 就绪超时，恢复 main 的完整 CI。不得提高预算或删除失败断言。
2. **业务语义与质量**：在独立用例中共同约束主链方向、域/子域包含、分支汇合和路由净空；通过后接入默认泳道，再独立提交。目前工作区的全局流程轴原型尚未通过 TB/LR 的完整连线质量，不纳入稳定修复提交。不得重复进行无证据的端口枚举或间距试探。
