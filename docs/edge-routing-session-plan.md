...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地。四图各 16 项布局已有历史连线检查覆盖，指定短视口跨引擎序列已通过；四图指定主链 TB/LR/BT/RL/TB 共 20 次浏览器语义验收通过。`a085c4d0` 和 `7145a275` 完整 CI 均成功。普通自动保存后刷新质量拒绝及菜单方向丢失已完成本地公共修复，Logistics 最终 production 保存→刷新→继续 TB/LR 切换通过；本批远端完整 CI 仍待验证。正式性能、更多保存图、剩余业务语义/任意跨引擎切换及导出完整验收仍未完成。
