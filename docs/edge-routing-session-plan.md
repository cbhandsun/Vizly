...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；`515bda3c` 的远端完整 CI 已通过。Logistics 复杂布局的外圈返回通道缺陷已修复，并补齐复杂/树形八项最终连线检查；结合此前证据，四图各 16 项布局均已有连线检查覆盖。高级子菜单越出视口、正式性能、跨域语义、跨引擎连续切换和导出验收仍未完成。
