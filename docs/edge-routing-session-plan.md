...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；`3ccc47f0` 的远端完整 CI 已通过。高级子菜单修复已推送为 `19c1557f`，半像素碰撞与泳道越界修复已推送为 `edf7f3a4`。四图各 16 项布局均已有历史连线检查覆盖；Demand/TMS/WMS 的短视口跨引擎各四步通过，最新标签避障修复也补齐 Logistics 同序列。正式冷路由性能、跨域语义、保存图/任意跨引擎切换和导出完整验收仍未完成。
