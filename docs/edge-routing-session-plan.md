...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；反向布局批次 `722ce563` 的远端完整 CI 已通过。Demand/TMS/WMS Process 已覆盖 16 种布局的最终连线检查，Logistics 复杂布局仍有稳定复现的质量失败；正式性能、跨域语义、跨引擎连续切换和导出验收仍未完成。
