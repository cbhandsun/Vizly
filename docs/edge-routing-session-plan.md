...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地。四图各 16 项布局均已有历史连线检查覆盖；四图短视口跨引擎指定四步序列也已通过。最新已推送 `180c2b9e` 的完整 CI `33422533357` 成功；性能专项 `33422533369` 增量与交互通过、冷路由失败。候选预算减算已完成本地发布验证，尚不代表性能达标。正式冷路由性能、跨域语义、保存图/任意跨引擎切换和导出完整验收仍未完成。
