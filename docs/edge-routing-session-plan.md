...
状态：质量、会话、拓扑、缺陷调度与 standalone 渲染协议主链已落地；端点穿节点修复已推送 main `75f653df`。该批远端静态/构建/smoke 及四组测试通过，但 routing 组仍有 WMS production LR 超时，不能记为完整 CI 通过。WMS Demand BT、性能专项和最终完整矩阵尚未闭环。
