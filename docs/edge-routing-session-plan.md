...
## 49. 泳道组级紧凑包络（下一批，保持等宽/等高）

用户反馈 WMS Demand Allocation 的泳道整体被拉得过长，同时明确同一泳道组内必须保持同宽或同高。后续实现不做逐泳道独立收缩，而采用组级尺寸模型：横向泳道共享统一宽度，纵向泳道共享统一高度；共同长度由组内全部可见业务节点、标题区、内边距和跨泳道连线所需的有界路由走廊共同决定。外部非成员节点、其他泳道组和远端大容器不得无条件扩大本组长度，人工明确放大的尺寸则按现有持久化语义保留。

该方向与常见的 cross-functional grid/table 模型一致：泳道是相邻的一维分区，组级行列保存统一几何与最小尺寸，内容节点留在所属分区；自动布局负责内容包络，交互仍允许调整整组。验收必须同时证明：同组泳道目标轴尺寸完全一致、所有子节点位于所属泳道内、标题和尾部留白有界、跨泳道边保持 hard-clean、折叠/展开与保存恢复稳定、TB/BT/LR/RL 四方向及空泳道不退化。优先级排在生命周期批次之后、低收益冷路由微调之前；先修截图中的生产样本并提炼通用回归，不按图 ID 特判。
