...
术语必须保持单义：**dual-trunk edge（双主干边）** 专指同一条业务边同时属于 source trunk 与 target trunk 的情况；**crossing bridge / line-hop（交叉跳线）** 专指两条线在交点处不发生拓扑连接时使用的 jump/gap 渲染语法。交叉可以是可读性权衡后的合理选择，不要求先证明其数学上不可避免。不得再用 `bridge edge` 表示双主干身份，也不得把交叉跳线写回真实路径拓扑。

**交叉标准修订（2026-09-05，依据用户确认与行业文档）**：普通连线可以交叉，目标是连接关系清楚、路径易于追踪，并在交叉、弯折、长度之间平衡。下文历史验证条目中的“交叉为零”“严格交叉不得增加”等阻断要求，统一限定为**未消解的歧义交叉或相关硬约束违规**，不包括满足“交叉分类与优化标准”的可读交叉；原始几何交叉仍须完整统计。节点避障、固定端口、端点附着、异常重叠和合法主干语义等独立要求继续有效。本次修订规定目标与验收口径，不代表现有实现、缓存和浏览器门禁已经完成迁移。

本轮落地范围与验证证据见 [连线约束模型修复记录](edge-routing-balanced-model-report.md)，其中区分了已通过的企业图验收与尚未覆盖的完整发布矩阵。
...
## 布局与连线的联合提交约束（2026-09-06）

显式自动布局必须同时满足下列条件，不能用路由 `hardClean=true` 替代布局验收：

- 节点尺寸与坐标有限、有效；可见业务节点互不重叠，真实父子关系有效，子节点位于父容器内。祖孙容器的合法包含与无关容器重叠分别处理，不能只靠背景视觉覆盖推断归属。
- 泳道严格按指定方向对齐并具有统一流程长度；明确的域顺序和布局前的节点归属必须保留。没有指定顺序时，允许算法选择域顺序，但路由阶段不能再改变这一结果。
- 非泳道域、子域与域直属节点可以采用分层、grid、流式或独立分量组合；有依赖的流程层级应保留。候选在父级可用空间内比较，不能仅凭局部面积减少认定整图改善。
- 节点／层次／约束摘要与本次最终路由摘要绑定，在同一任务版本中一次验收并发布节点、边和布局选择。无边图也必须通过布局验收；缓存命中和 Worker 收据不豁免这一步。过期任务、计算期间被修改的几何和不完整组候选不得发布。
- 隐藏、折叠与实际渲染采用一致的可见性定义；布局适配器负责同步新坐标、真实父关系和尺寸，不能把旧测量值当成新布局结果。
- 组协同候选必须同时满足该组端口角色、声明通道占用、既有主干和全图硬约束。通道预留不能只作为“曾经建议过”的坐标提示；失败时不部分提交。搜索保持有界，不通过提高超时或扩大预算代替约束建模。

概览视图按节点、实际路径与标签范围显示全图；阅读视图单独保留可读缩放。缩放与导出的最终可见效果仍须浏览器验收。上述是提交和验证标准，实现范围与本轮验证记录见 [统一修复记录](edge-routing-normal-entry-fix.md) 和 [布局空间优化](layout-space-optimization.md)。
...
2. **少交叉，但不强求零交叉** — 清晰的普通直角交叉属于可读性成本，不是几何失败。共享节点的分支交叉应赋予更高成本，但不能仅凭 buddy 身份一律禁止；合法共同主干与“相交但不连接”必须分别表达
3. **短路径与少弯折共同优化** — 连线尽量短、转弯尽量少。允许为了避障、合法共享主干或明显改善可读性走外侧通道；不得仅为消除一个清晰交叉而制造巨大外绕、连续台阶或端口反向
...
9. **局部直连优先** — 近距离、近似同轴的父子链路不应为了消除一次清晰线线交叉而产生巨大绕行；当直连满足节点、端口和主干约束，且交叉能清晰表达为不连接时，可以保留并计入可读性成本。这是一般策略，不限于某个模板、bus 标记或边数阈值
...
3. **外侧绕行可读** — 当内侧通道会穿节点或形成难以辨认的拥挤关系时，比较稳定的外侧通道；单纯存在普通交叉不足以强制外绕

## 🟨 交叉分类与优化标准

这些标准用于区分“线线相交”“错误重叠”和“连接歧义”。几何上的 strict crossing 只表示交点落在线段有效内部，不自动决定严重级别。主流工具提供的是交叉最小化、成本权衡和桥接表达；以下来源没有给出适用于所有图的“每条边最多 N 次交叉”或“全图必须零交叉”标准。

| 情况 | Vizly 的处理原则 |
| --- | --- |
| 空白区域内、能够清楚显示不连接的普通直角交叉 | 允许；保留交叉计数，纳入可读性成本 |
| 共享节点的分支互相交叉 | 更高优化优先级；先比较端口顺序、tap 与主干候选，不能直接当作连接点或无限制强制消除 |
| 靠近端口、箭头、标签、拐角、主干 junction 的交叉，或密集交叉簇 | 高风险可读性问题；优先重路由或调整表达，无法清晰表达不连接时阻断最终展示 |
| 无关线共线重叠、反向重叠、未解释的共同段、误接端点、穿业务节点 | 独立硬错误；不能用普通交叉许可豁免 |
| 有明确 source/target/bus 语义的共同主干及 tap | 合法拓扑连接；按主干和 junction 规则表达，不画成非连接跳线 |

1. **普通交叉采用有限成本** — 不把交叉数作为压倒所有长度和弯折收益的绝对目标。先满足硬约束，再比较交叉成本、弯折、长度、端口顺序、主干结构与增量稳定性。相近路径中优先少交叉；明显更短、更少弯折且同样清楚的路径可以保留交叉。
...
6. **交叉点位置也要评分** — 保留的交叉应远离节点边界、端口 stub、标签、bus junction 和短 dogleg，并为实际跳线/断口留出空间。距 bend 仅 `0.5-1px` 的相交仍须被几何检测完整发现，再判断其歧义和渲染可行性；不得用扩大 EPS 隐藏交点。只有明确的拓扑接点才能解释为连接，几何端点接触本身不是连接证明。
7. **buddy 内部交叉不等于共同主干** — 同一 group 内的垂直穿插也要表达为“相交但不连接”，并施加比普通交叉更高的成本；可以通过合并 junction、借用同组后缀或重排 tap 改善。只有合法共线公共段才能按 trunk 共享，不能因两边同源/同目标就把任意交点变成 junction。
8. **先换拓扑，再做 nudging** — 对 bus 交叉问题，优先通过 trunk lane、junction 顺序、共享后缀/前缀修复拓扑；nudging 只用于分离可辨间距，不应作为修复错误 bus 拓扑的主要手段。
9. **桥接表达必须真实可见且稳定** — 统一使用 jump/arc 或 gap 表示不连接，采用稳定的承载方向/优先级；不得在普通交叉处画表示连接的实心 junction。密集交叉可以合并为覆盖整簇的桥接，也可以重路由，不能省略其中部分交叉的表达。默认态、hover/focus/selected、缩放和导出应表达同一连接关系。
10. **同时保留原始计数与分类计数** — 至少区分几何交叉总数、可读交叉数、歧义交叉数和相邻边交叉成本。原始总数不能因为加了跳线或降低了严重级别而消失；算法评价、增量评价、缓存、Worker 报告及最终 SVG 必须使用一致的分类。单个 `h` 标记、声明或布尔值不能代替真实几何与可见桥接证据。
11. **不设置无依据的统一交叉次数上限** — 按图规模、局部密度、每条路径的追踪难度和主流程重要性评估。少量孤立交叉与相同数量挤在一个端口附近的交叉应得到不同评价；若设项目阈值，必须标明用途与验证样本，不能声称是通用行业标准。
12. **成本和渲染尺寸分别校准** — yFiles 当前默认普通交叉成本为 `1`、弯折成本为 `3`、相邻边交叉成本为 `7`；yEd 另提供以弯折次数衡量的交叉成本，推荐 `1–3`。两者使用各自的评分体系，不能混作统一单位或直接当作 Vizly 已验收的参数。Vizly 既有 `6px` 跳线半径、`24px` 交点至 bend/terminal 的候选安全余量属于本项目渲染参数，应结合圆角、箭头、标签和缩放验证，不是行业规定。过高交叉惩罚可能增加绕路和求解耗时，不能用不断加大惩罚或重复修复追求零交叉。

13. **交叉判定必须包含主干接点与最终绘制边界** — 两条原始路径的交点远离各自折点，并不足以证明跳线可见。交点若同时落在第三条双主干边的 source 分叉与 target 汇聚处，绘制主干时可能把承载跳线的水平段截断，造成虚假连接。此类接点需要分开分叉与汇聚位置，为双主干边保留可辨识的中间通道；必要的主干移位按整组事务完成，继续保留两端主干身份、固定端口与节点净距。验收以实际可见 SVG/导出片段为准，不能只检查原始 `computedPath`。这是 Vizly 对最终连接语义的工程约束，不是新增的行业统一像素标准。

14. **用可行候选验证权衡，不把单边比值当作全图目标** — 绕行比用于定位可读性风险，不能单独证明当前路线较差。比较候选时先检验全图节点净距、端口、主干及重叠，再按统一成本比较交叉、弯折与长度。测试应保留实际候选及比较证据；当“单条线必须更短”与全图可读性成本冲突时，不能通过提高硬约束阈值或增加无限搜索来满足该断言。确定只剩净距等单类问题时，先使用该问题的有界事务修复，再考虑整图重算。
...
10. **保留的非连接交叉需要 crossing bridge / line-hop 语法** — 允许保留的交叉必须用统一的 jump/bridge/line-hop/gap 表达“经过但不连接”，并规定稳定的优先级；不能让两条实线直接相交后要求用户猜测连接关系。无需证明交叉不可避免，但必须验证其可辨认性。crossing bridge / line-hop 只属于交叉处的渲染层语法，不能用于命名 dual-trunk edge，不能改变真实连接拓扑，也不能遮住附近箭头、标签或 junction。
...
| 保留交叉的桥接表达 | `100%` 的非连接交叉具有清晰、一致的 jump/bridge/line-hop/gap 表达 | 普通实线直接相交且关系不明，或密集交叉漏画 | 按实际可见几何验收；不得表达 dual-trunk identity |
| 视觉语义字段保真 | 合法显式字段 `100%` 保留 | 任一转换分支静默丢失或被 fallback 覆盖 | 对比输入、canvas edge、Worker patch、最终 DOM computed style 与导出结果 |

## 优先级排序

**先满足硬约束和可辨认性，再平衡软成本。** 正交、真实端点连接、显式 strong/fixed/forbidden 端口、业务节点避障、异常重叠控制和合法 source/target/dual-trunk 语义不能被分数抵消；交叉的桥接表达、追线反馈和导出一致性按对应质量层独立验收。

通过上述条件后，在**普通交叉、相邻边交叉、端点弯折、总弯折、长度、端口/通道顺序、增量稳定性**之间使用有界的成本权衡。不能把“减少一次普通交叉”永久排在“减少任意数量的弯折与绕行”之前；也不能用综合分数掩盖未消解的连接歧义。
...
20. **密集交叉采用有界链式 junction** — 当一条内部段依次穿过多根相邻主干，普通外移会穿节点、而直接绕单个交点会产生 `<24px` 小段时，可以沿行进方向把交点改成单调 junction 链：每次只沿阻塞主干跟随 `24px`，再进入下一条正交车道。入口、相邻 junction 和出口间距必须为 `0` 或 `>=24px`；整条候选仍须满足零未消解歧义交叉、零异常 overlap、零 tiny dogleg、零节点命中。这个 `24px` junction 是消除交叉的局部拓扑，不得被扩展成无关边的长共享主干。
...
34. **同轴拓扑锁采用双 stub 外侧包络兜底** — 当一条 `H→H` 或 `V→V` 边与无关反向主干形成长 overlap，且平移单段必然让两条接腿产生新严格交叉时，常规局部候选失败后才可惰性生成整边外侧包络候选。候选必须保留首尾原轴向和既有 endpoint stub，外侧 lane 只能从附近真实障碍矩形的上下/左右包络加安全间距推导，不能写死边 ID、图坐标或无界 delta。生成数量、路径点和参与节点必须有明确上限；最终仍须同时满足零未消解歧义交叉、零错误 overlap、零障碍命中、零 tiny dogleg/hairpin 和 stub 硬下限，不能以远距离绕行换取单项清零。
...
30. **增量分层安全距离回归** — 至少覆盖未出现在 `contextEdgeIds` 的冻结分支在 changed node 移动后进入 `<48px` 区域、恰好 `48px` 不提升、incident/mutable 边不重复提升、超过八条需提升时回退完整路由，以及 TMS/WMS/L-OMS 的大位移与吸附后小位移。断言被提升分支进入完整 transaction；安全候选存在时恢复 `48px`，端口几何无解时仍须满足 `>=16px`、零节点命中、零未消解歧义交叉、端点 anchored，并保持 incremental commit，不能退化成整图跳转。
...
54. **逆向线独立交叉回归** — 至少构造“逆向 through lane 穿过无关正向 source bus”“逆向边合法汇入同目标 trunk”“逆向边同时属于另一端真主干”三类场景；前者必须重路由或桥接且不得生成 junction，后两者只能在公共 stem 边界生成一个 owner junction。Logistics 正式图需逐段断言 DATA→下游逆向竖线与 DATA 正向汇入主干间距 `>=48px`、与全部无关正向横线未消解歧义交叉为零，并锁定其顶部同目标 target-trunk junction；端口、节点避障、dual membership 和导出像素同步验证。
...
63. **同源短分支应优先借用安全兄弟主干** — 当 O2M 中一条短分支的内部横段切穿另一条同源边的竖向 spine 时，不能把它当作共享端点接触放行，也不应直接把分支送往全图外圈。对 `free/weak/auto` 端口，应有界枚举“切换到兄弟 source side → 精确复用其同向公共前缀 → 在目标投影处形成一个正交 tap → 沿原 target side 接入”的原子候选；只有整条候选同时满足逐边节点命中为零、未消解歧义交叉为零、异常 overlap 为零、端点 attached/anchored、既有 source/target/dual true trunk 不缩短时才能提交。拓扑接点豁免只适用于两边在同一真实 source/target 点形成的分叉或汇聚；内部 segment 相交不能冒充 junction，但满足交叉分类标准时可作为非连接交叉保留并计入成本。回归必须包含短分支与长兄弟 spine、兄弟主干本身兼具 target trunk 身份、人工 fixed 端口拒绝切换，以及候选顺序打乱后的确定性结果。
...
74. **大图候选笛卡尔积必须在实体化前限界** — 端口、终端轴、外侧 lane 与 trunk 的组合搜索，在 `>24` 条边时不得先展开全部 `lane × trunk` 再排序截断；应先按当前端点/主干距离保留最近坐标，同时保留两侧全局极值以维持真正 outer-skirt 逃逸能力，再生成候选。显式 full-quality 只表示不能降级为 fast 质量，并不解除 bounded seed 和候选预算；force 标志必须从 Worker 请求贯穿 seed 与 quality budget。小图保留完整搜索，大图回归同时断言零未消解歧义交叉、零异常 overlap、零节点命中、端点 attached/anchored、真主干不退化与冷路由预算，不得通过降低质量或提高超时换取性能。
...
- **端口与通道共同决策**：无既有路径时，按手工端口约束枚举可用侧，并同时检查向外退出、节点避障和目标净距，再比较弯折与长度。不得先以默认 DOM 端口绘制路径、再独立修补 handle。路径端点与 sourceHandle/targetHandle 必须作为同一候选提交。
- **构造和修复使用同一净距目标**：障碍绕行候选应使用商业可读性净距（当前 48px）；8px 物理碰撞检测 padding 不能充当最终通道间距。48px 是 Vizly 产品选择，不是统一行业标准。
- **初始构造与既有路径优化分开**：新图、恢复数据或布局后无 computedPath 的输入先构造可用路线，再进入一次完整事务；已有可信路线继续增量优化。候选构造有明确规模上限，不能无限枚举端口与通道，不能在 UI 主线程运行重型寻路。
- **临时路径不等于可用路线**：布局生成的 computedPath 和运行时端口锁只记录临时决策。初始路线的受阻边数超过既有交互修复额度时，应进入构造流程，不能仅凭“受阻比例不到半数”继续局部修补；手工路线及其他引擎的已有拓扑仍须保留。
- **原请求身份不变**：路由初始化只产生 Worker 内候选，不改变请求的输入摘要、业务边或手工端口声明；最终结果继续经过全图质量门禁和原子提交。
- **用户操作才是验收入口**：必须覆盖普通页面恢复，以及“标准流程 → 泳道布局 → 刷新”的持久化链路；使用原有 12 秒交互截止时间。专用采集入口、已有路径种子和延长超时的结果不能替代此验收。合法桥接交叉允许保留，穿节点和端口失配不可接受。
...
- yFiles `EdgeRouterCosts`（2026-09-05 核对）：区分普通交叉、相邻边交叉、弯折、节点和端口违规成本；默认普通交叉成本 `1`、弯折 `3`、相邻边交叉 `7`，并说明过高成本可能显著增加求解时间与路由伪影。[官方成本模型](https://docs.yworks.com/yfiles-html/api/EdgeRouterCosts/)
- yEd `Orthogonal Edge Router`（2026-09-05 核对）：交叉成本 `n` 表示愿意为避免一次交叉增加约 `n` 次转向，推荐权衡范围 `1–3`；另提供局部交叉最小化与重路由选项。[官方路由说明](https://yed.yworks.com/support/manual/layout/router_orthogonal.html)
- yFiles `BridgeManager` 与 GoJS `JumpOver` / `JumpGap`：为线线相交提供弧桥或断口，表达经过但不连接；桥接是独立的渲染能力，不以全图零交叉为前提。[yFiles 桥接](https://docs.yworks.com/yfiles-html/api/BridgeManager/)；[GoJS 连线](https://gojs.net/latest/learn/links)
...
## 布局空间与连线约束（2026-09-06 确认）

- 泳道模式严格遵守泳道边界对齐、统一流程方向长度、域顺序和归属；压缩应在这些约束内进行。
- 非泳道模式允许内容自适应分组、分块填空和更自由的排布，不继承严格等长泳道约束。
- 子域内部可以采用 grid、流式换行、分层或独立分量组合，不必强制单行／单列；明确的流程依赖仍须保留。
- 不通过缩小文字、节点内容或既定连线安全距离换取面积收益。压缩后的端口、避障和交叉表达继续接受同一组路由门禁。
- 优化需检查整图宽高、面积与阅读比例；单个域的面积变小不能替代整图验证。

方法、行业依据和前后测量见 [布局空间优化](layout-space-optimization.md)。

## 检查输出建议

检查器输出建议分为三类，避免把视觉问题混进硬错误里导致误修：

1. `error` — 违反硬约束：非正交、端点非正交进出或逐角色 attached/anchored 退化、真实节点穿越、反向/无关/未解释 overlap、`<24px` tiny interior dogleg、hairpin、共享主干被破坏。不能仅因普通几何交叉存在而输出硬错误。
2. `warning` — 视觉软约束风险：容器视觉穿越、节点近距、边间距不足、绕远异常、方向单调性破坏、标签冲突、默认态对比度不足、标签归属不明、LOD 密度过高、主干重复叠画或缺少交叉桥接。
3. `info` — 合法且纳入可读性评价：具有清晰桥接表达的交叉、允许的 O2M/M2O 主干重叠、为避障而走外侧通道、为改善可读性而增加长度。保留交叉须同时报告原始数量和对应成本；无需逐次证明其不可避免。
...
1. **孤立性按完整业务图判断。** 子域内没有边，不代表节点可以任意装箱；跨域连接、经域外节点相连的局部成员，仍受全局流程和通道关系约束。自动、Grid、Flow、域直属内容、嵌套子域及后续重排必须使用同一全图连接关系。局部排布只能过滤本次要放置的节点，不能丢弃判断其约束所需的外部拓扑。真正独立的内容继续利用空白区域。
2. **同一物理端口侧的入线和出线共同占用空间。** 不能只按 source 主干或 target 主干分别修复。存在互相阻挡时，应将完整同侧角色组作为候选一起移动；固定位置锁定整个受影响单元，固定侧允许侧内移动。保持远端端点、同侧次序、有效共享主干及冻结边界，并在既有预算内对整组候选执行全图验收。
3. **压缩收益与路由可行性分别举证。** 节点包围盒变小、布局几何通过、离线候选通过，均不能替代普通首开和实际布局切换的成功提交。生产验证必须包括全新浏览器状态、已有图的模式切换和保存后的重载；超时必须在无其他重型计算竞争时复验，不能增加截止时间来接受空间优化。
4. **所有概览入口采用同一可用区域。** 初始适应、布局提交后适应、手动 Fit 和允许的 resize 适应，共用实际节点／路径／标签范围和最终侧栏安全区。自动任务不能覆盖较新的保存视角或用户手势；预览期间的临时交互状态不能改变最终测量区域。

本节是 Vizly 的工程约束，不把项目选定的间距数值描述为所有行业产品必须采用的统一标准。实现状态与真实验证结果记录在 [统一修复记录](edge-routing-normal-entry-fix.md)。
